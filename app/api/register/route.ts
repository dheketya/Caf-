  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const candidate = `${abbr}${suffix}`
        const existing = await tx.shop.findUnique({ where: { shopCode: candidate } })
        if (!existing) { shopCode = candidate; break }
      }
      if (!shopCode) shopCode = `${abbr}${Date.now().toString(36).slice(-4).toUpperCase()}`
